...
```bash
curl -sSL https://raw.githubusercontent.com/Mist-wu/sub2api/main/deploy/install.sh | sudo bash
```
...
# Uninstall
curl -sSL https://raw.githubusercontent.com/Mist-wu/sub2api/main/deploy/install.sh | sudo bash -s -- uninstall -y
```
...
curl -sSL https://raw.githubusercontent.com/Mist-wu/sub2api/main/deploy/docker-deploy.sh | bash
...
git clone https://github.com/Mist-wu/sub2api.git
cd sub2api/deploy
...
Sub2API supports [Antigravity](https://antigravity.so/) accounts. The production catalog is intentionally conservative and only exposes Gemini preview request names.
...
For production accounts, keep `model_mapping` explicit. An empty mapping means all default models are allowed.
...
Antigravity accounts support optional **hybrid scheduling**. When enabled, the general endpoint `/v1beta/` will also route Gemini requests to Antigravity accounts.
...
<a href="https://star-history.com/#Mist-wu/sub2api&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=Mist-wu/sub2api&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=Mist-wu/sub2api&type=Date" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=Mist-wu/sub2api&type=Date" />
 </picture>
</a>
